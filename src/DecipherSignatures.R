...
DecipherSignatures <- function(input_file, 
                               output_dir,
                               k_mer,
    ...
    if(!dir.exists(paste0(output_dir,'/temp'))){
        dir.create(paste0(output_dir,'/temp'))
        # suppressWarnings(
        #     fr <- do.call(file.remove, list(list.files(paste0(output_dir,'/temp'), full.names = T)))
        # )
    }
    ########################################
    ...
    if(k_mer == 5){
        ...
        selected_rows <- which(sapply(M$motif, function(m) any(sapply(selected_motifs, function(sm) grepl(sm, m)))))
        if(length(selected_rows) == 0){
            stop('Selected motifs did not match to any 3-mer motifs',call. = F)
        }
        M <- M[selected_rows,]
    }
    #-------------------------
    ...
    M <- matrix(M, ncol = G)
    remaining_mut_types <- setdiff(c(1:num_motifs),inds)  # mutation types that are present after step 1
    ...
    cat('\n')
    cat(paste("With",as.numeric(number_of_CPUs),"CPU cores\n"))
    ...
    for(N in c(N_min:N_max))
    {
      ...
      for(epoch_boot in 1:Boot_max_epoches)
      {
        ...
        if(epoch_boot != 1)
        {
          ...
          st_push <- Sys.time()
          fwrite(as.data.table(SP),paste0(output_dir,'/','temp/SP.tsv'),sep = '\t',row.names = F,col.names = F)
          fwrite(as.data.table(SE),paste0(output_dir,'/','temp/SE.tsv'),sep = '\t',row.names = F,col.names = F)
          rm("SP")
          rm("SE")
          se_push <- Sys.time()
        ...
        if(number_of_CPUs > 1)
        {
          ...
          # cl <- makeForkCluster(number_of_CPUs)
        ...
        if(epoch_boot == 1)
        {
          ...
          suppressMessages({
              suppressWarnings({
                  SP <- as.matrix(fread(paste0(output_dir,'/','temp/SP.tsv'),sep = '\t',header=F))
                  SE <- as.matrix(fread(paste0(output_dir,'/','temp/SE.tsv'),sep = '\t',header=F))
              })
          })
          file.remove(paste0(output_dir,'/','temp/SP.tsv'))
          file.remove(paste0(output_dir,'/','temp/SE.tsv'))
          se_pull <- Sys.time()
      ...
      write(c(N,avg_silh_widths,re.E),file=paste0(output_dir,'/',"Evaluation.txt"),append=(N!=1))
      ...
      P <- cbind(data.table(motif = motif_list), P)
      
      fwrite(P, paste0(output_dir,'/',k_mer,"mer_Signatures_(N=",as.character(N),").tsv"),sep = '\t', col.names = T, row.names = F)
      ...
      colnames(exposures) <- paste0('Exposure_to_Signature_No_',1:(dim(P)[2]-1))
      
      exposures <- cbind(data.table(Sample_ID = sample_ids), exposures)
      
      fwrite(exposures,paste0(output_dir,'/',"Exposures_to_",k_mer,"mer_Signatures_(N=",as.character(N),").tsv"),sep = '\t', col.names = T, row.names = F)
